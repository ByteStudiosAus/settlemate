...
test("GET /voice/payers sums correctly for payments generated by a native Plan+Subscription (real sandbox shape)", async () => {
  ...
  // duplicate payer records in the sandbox (see CLAUDE.md's "Pinch does NOT
  // enforce payer-name uniqueness" note) — this test locks in that the
  // overdue-walk itself handles the native-plan shape correctly.
  const env = makeEnv(baseGrant());
  const fx = stubFetch([
    {
      data: [
